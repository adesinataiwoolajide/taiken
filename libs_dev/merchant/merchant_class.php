<?php 
	...
	class customerOrders extends productMerchant{
		...
		public function getMerchProducts($merchant_number, $product_number)
		{
			$query = $this->db->prepare("SELECT * FROM products WHERE merchant_number=:merchant_number AND product_number=:product_number");
			$arrGetProDet= array(':merchant_number'=>$merchant_number, ':product_number'=>$product_number);
			$query->execute($arrGetProDet);
			$fetch = $query->fetch();
			return $fetch;
		}

		public function getMerchantProducts($order_id)
		{
			$query = $this->db->prepare("SELECT * FROM customer_order WHERE order_id=:order_id");
			$arrGetProDet= array(':order_id'=>$order_id);
			$query->execute($arrGetProDet);
			$fetch = $query->fetch();
			return $fetch;
		}
