<?php
	class ragzNationProductsCategory{
		...
		public function addingNewProductsType($type_name){
			try{
				$insertType = $this->db->prepare("INSERT INTO product_type(type_name)VALUES(:type_name)");
				$arrType = array(':type_name'=>$type_name);
		...
		public function checkDuplicateProductsType($type_name){
			try{
				$checkType = $this->db->prepare("SELECT * FROM product_type WHERE type_name=:type_name");
				$arrT = array(':type_name'=>$type_name);
		...
		public function updateType($type_name, $type_id){
			try{
				$update = $this->db->prepare("UPDATE product_type SET type_name=:type_name WHERE type_id=:type_id");
	...
	class ragzNationProducts extends ragzNationProductsCategory{
		...
		public function addNewProductDetails($product_name, $product_number, $product_image, $category_id, $manufacturer_id){
			try{
				$insertProDetails = $this->db->prepare("INSERT INTO products_details(product_name, product_number, product_image, category_id, manufacturer_id)VALUES(:product_name, :product_number, :product_image, :category_id, :manufacturer_id)");
				$arrProDet= array(':product_name'=>$product_name, ':product_number'=>$product_number, ':product_image'=>$product_image, ':category_id'=>$category_id, ':manufacturer_id'=>$manufacturer_id);
		...
		public function updateNewProductDetailsWithImage($product_name, $product_number, $product_image, $category_id, $manufacturer_id){
			try{
				$updateProDet = $this->db->prepare("UPDATE products_details SET product_name=:product_name, product_image=:product_image, category_id=:category_id, manufacturer_id=:manufacturer_id WHERE product_number=:product_number");
				$arrUpProDet= array(':product_name'=>$product_name, ':product_number'=>$product_number, 
					':product_image'=>$product_image, ':category_id'=>$category_id, ':manufacturer_id'=>$manufacturer_id);
		...
		public function updateNewProductDetailsWithoutImage($product_name, $product_number, $category_id, 
			$manufacturer_id){
			try{
				$updateProDet = $this->db->prepare("UPDATE products_details SET product_name=:product_name, category_id=:category_id, manufacturer_id=:manufacturer_id WHERE product_number=:product_number");
				$arrUpProDet= array(':product_name'=>$product_name, ':category_id'=>$category_id, ':manufacturer_id'=>$manufacturer_id, ':product_number'=>$product_number);
				if(!empty($updateProDet->execute($arrUpProDet))){
					return true;
				}else{
					return false;
		...
		public function gettingAllOrders()
		{
			try {
				$query = $this->db->prepare("SELECT * FROM customer_order_details ORDER BY id desc ");
				$query->execute();
				$result = $query->fetchAll(PDO::FETCH_ASSOC);
				return $result;
			}catch(PDOException $e){
		...
		{
			$query = $this->db->prepare("SELECT * FROM products WHERE product_price <=:product_price ORDER BY product_price ASC");
